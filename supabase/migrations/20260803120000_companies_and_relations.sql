/*
# Companies (Empresas) + Multi-tenant Collaborators

Adds multi-company support to ManuGent:
- `companies` table for tenant-like organisation
- `users.company_id` FK → companies
- `clients.company_id` FK → companies
- RPCs: get_companies, get_users_with_company, get_company_collaborators, get_clients_with_company
- Seed demo companies with credentials

This is the Supabase mirror of docker/postgres/init/007_companies_and_relations.sql
(with RLS policies for Supabase roles).
*/
...
-- ── RLS policies (permissive — app handles auth layer) ───────────────────

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    EXECUTE 'CREATE POLICY "anon_all_companies" ON companies FOR ALL USING (true) WITH CHECK (true)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    EXECUTE 'GRANT EXECUTE ON FUNCTION get_companies() TO anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION get_users_with_company() TO anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION get_company_collaborators(uuid) TO anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION get_clients_with_company() TO anon, authenticated';
  END IF;
END $$;
...
-- Ensure existing superadmin/admin/gestor/tecnico demo accounts remain global (no company)
