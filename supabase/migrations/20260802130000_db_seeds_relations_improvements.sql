/*
# Melhorias na Base de Dados, Seeds e Relações do Sistema

Estende o schema para cobrir os módulos pedidos que ainda não existiam
como tabelas reais (apenas como módulos de permissões no frontend):

1. `users` (Colaboradores) — cargo, departamento, telefone, avatar
2. `floors`, `areas` — hierarquia Edifício → Piso → Área, e equipment
   passa a poder referenciar piso/área (aditivo)
3. `building_assignments` — colaboradores responsáveis por edifício
   (responsável, gestor, supervisor, equipa de manutenção, administrativo)
4. `incidents`, `incident_comments` — incidentes com estado, prioridade,
   responsável, fotografias (jsonb) e ligação a OT
5. `knowledge_categories`, `knowledge_articles`, `knowledge_article_versions`
   — base de conhecimento com categorias, etiquetas, anexos e versões
6. `quotes` (Orçamentos) — building_id, desconto/imposto, `quote_items`
   (materiais/mão de obra) e `quote_status_history`
7. `parts` — unidade de medida; `inventory_movements` — histórico de
   movimentos de stock
8. `purchase_orders`, `purchase_order_items`, `purchase_order_approvals`
   — compras associadas a fornecedores, materiais e centro de custo
9. `calendar_events` — building_id/client_id/equipment_id/priority/status
   e novos tipos de trabalho agendado (inspeção, reparação, visita
   técnica, instalação), para cobrir "Trabalhos Agendados"

Tudo aditivo (ADD COLUMN IF NOT EXISTS / CREATE TABLE IF NOT EXISTS) —
não quebra dados existentes. RLS segue o mesmo padrão permissivo já usado
no resto do schema (a app tem a sua própria camada de auth por cima).
*/
...
CREATE POLICY "anon_all_floors" ON floors FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_building_assignments" ON building_assignments FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_incident_comments" ON incident_comments FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_knowledge_categories" ON knowledge_categories FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_knowledge_articles" ON knowledge_articles FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_knowledge_article_versions" ON knowledge_article_versions FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_quote_items" ON quote_items FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_quote_status_history" ON quote_status_history FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_inventory_movements" ON inventory_movements FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_purchase_orders" ON purchase_orders FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_purchase_order_items" ON purchase_order_items FOR ALL USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_all_purchase_order_approvals" ON purchase_order_approvals FOR ALL USING (true) WITH CHECK (true);

ALTER TABLE inventory_movements
  ADD CONSTRAINT inventory_movements_po_fk FOREIGN KEY (related_purchase_order_id) REFERENCES purchase_orders(id);

-- link de documentos anexos a uma compra (reutiliza a tabela documents existente)
