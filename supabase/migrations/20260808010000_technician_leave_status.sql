/*
# Estados de ausência de colaboradores (Folga, Doença) + motivo/regresso

1. Schema changes
   - `technician_profiles.status` — passa a aceitar também 'folga' e
     'doenca', além de 'disponivel', 'em_servico', 'ausente', 'ferias'.
   - `technician_profiles.status_note` — motivo livre (ex. "Formação
     externa", "Atestado médico", "Dia de folga"), mostrado na vista
     "Equipa do dia" do Calendário quando o colaborador não está a
     trabalhar.
   - `technician_profiles.status_until` — data de regresso prevista
     (usado sobretudo para férias, ex. "Retorno: 18/08/2026").

2. get_technicians() — passa a devolver status_note/status_until, e o
   label de 'em_servico' muda de "Em campo" para "A trabalhar" para
   alinhar com a vista "Equipa do dia".
...
GRANT EXECUTE ON FUNCTION create_technician(text, text, text, text, text, text, uuid, uuid, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION update_technician(uuid, text, text, text, text, text, text, uuid, uuid, text) TO anon, authenticated;

-- ── get_technicians(): devolver também motivo/data de regresso ─────────────

DROP FUNCTION IF EXISTS get_technicians();

CREATE FUNCTION get_technicians()
RETURNS TABLE (
  id uuid,
  name text,
  email text,
