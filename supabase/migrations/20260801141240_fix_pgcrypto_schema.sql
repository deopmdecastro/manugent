/*
# Fix verify_user_password search_path for pgcrypto

pgcrypto is installed in the "extensions" schema on Supabase, not "public".
This migration recreates all SECURITY DEFINER functions with search_path
including the "extensions" schema so crypt() and gen_salt() are found.
*/

-- Fix verify_user_password
...
-- Fix create_work_order_findings
...
-- Also re-seed the password hashes using extensions.gen_salt
UPDATE users SET password_hash = extensions.crypt('Demo@2026', extensions.gen_salt('bf', 10))
WHERE email IN ('superadmin@manugent.pt', 'admin@manugent.pt', 'gestor@manugent.pt', 'tecnico@manugent.pt', 'cliente@demo.pt');