-- 006_full_demo_dataset_schema.sql
-- Extende o schema local (Docker/PostgreSQL standalone) com todas as entidades
-- que o seed de dados fictícios (`scripts/seed-demo-data.mjs`) e a API consomem.
--
-- Espelha supabase/migrations/20260802120000_full_demo_dataset_schema.sql, mas
-- adaptado ao Postgres standalone: sem ROW LEVEL SECURITY e sem políticas para
-- as roles `anon`/`authenticated` (que só existem no Supabase). A app tem a sua
-- própria camada de auth (JWT) por cima.
