...
export function Topbar({ collapsed, user, onLogout, onToggleSidebar }: TopbarProps) {
  ...
  return (
    <header className="topbar">
      ...
      <button className="ai-button" onClick={() => window.location.hash = '#ai'}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {userMenuOpen && (
          <>
            ...
            <div className="glass-card" style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', width: 220, padding: 8, zIndex: 40, animation: 'fadeInUp 0.2s ease' }}>
              <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border)', marginBottom: 4 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{user?.name}</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{user?.email}</div>
              </div>
              <button className="sidebar-link" onClick={() => { window.location.hash = '#user-dashboard'; setUserMenuOpen(false); }} style={{ width: '100%' }}>
                <i className="fas fa-user-gear" /> <span>Painel</span>
              </button>
              <button className="sidebar-link" onClick={() => { window.location.hash = '#settings'; setUserMenuOpen(false); }} style={{ width: '100%' }}>
